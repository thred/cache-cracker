...
describe("Parser (Quantity)", () => {
    ...
    it("pass a quantity: 3 / 2 m", () => {
        let expression = Parser.parseExpression(Parser.scan("3 / 2 m"));

        assert.equal(expression.describe(), "3 / 2 m");
        assert.equal(expression.invoke().toString(), "1.5 m");
    });

    it("pass a quantity: 3 * 0.5 m", () => {
        let expression = Parser.parseExpression(Parser.scan("3 * 0.5 m"));

        assert.equal(expression.describe(), "3 * 0.5 m");
        assert.equal(expression.invoke().toString(), "1.5 m");
    });
    ...
    it("subtraction of lengths: 1 m - 50 cm", () => {
        ...
        assert.equal(expression.describe(), "1 m - 50 cm");
        assert.equal(expression.invoke().toString(), "0.5 m");
    });
    ...
    it("multiplication of a length and an area: 1 m * 50 cm²", () => {
        ...
        assert.equal(expression.describe(), "1 m * 50 cm²");
        assert.equal(expression.invoke().toString(), "0.005 m³");
    });
    ...
    it("angle: 45 ° 60 ' 3600 \"", () => {
        let expression = Parser.parseExpression(Parser.scan("45 ° 60 ' 3600 \""));

        assert.equal(expression.describe(), "45 ° 60 ' 3600 \"");
        assert.equal(expression.invoke().toString(), "47 °");
    });
